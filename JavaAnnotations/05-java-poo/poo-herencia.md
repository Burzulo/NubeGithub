...
La herencia es un mecanismo que permite definir **nuevas clases basadas en una clase existente**, es una **relación entre clases**. Esta permite definir una jerarquía de las mismas.
...
Esta NUEVA CLASE es denominada generalmente como ``subclase`` **(clase hija) puede poseer atributos y métodos que no existen en la clase original**. Por otro lado, la clase original es conocida como ``superclase`` **(clase padre/madre)**. Al igual que la «herencia genética» en la vida real, donde los hijos heredan ciertas características y comportamientos de los padres, en la «herencia de programación» se cumple exactamente el mismo concepto.
...
> 💡 **NOTA**  
> La herencia **permite reutilizar código**, ya que permite **compartir** automáticamente **«métodos»** y datos entre clase, subclases y objetos.

### 🔖 `extends`

La herencia a través de ``extends`` sigue una regla de **herencia simple**, lo que significa que una clase hija solo puede tener un único padre directo.
...
Palabra clave que permite acceder a métodos y atributos de la clase padre desde la subclase. Solo tiene sentido cuando **HAY HERENCIA**.

En tiempo de ejecución, el objeto es el mismo, pero ``super`` cambia el punto de vista para que actúes sobre el comportamiento heredado, incluso si está sobrescrito.
...
La anotación ``@Override`` es opcional pero recomendada. Le dice al compilador que estás sobrescribiendo un método. Si cometes un error (por ejemplo, mal nombre), el compilador te lo advierte. Por eso usar ``@Override`` **SIEMPRE** que sobrescribas un método. Evita errores sutiles.

<br>

Ampliando un poco mas, la anotación ``@Override`` cumple dos funciones principales que garantizan la solidez del código:
...
Ocurre cuando una clase tiene **varios métodos con el mismo nombre**, pero con **diferentes parámetros e implementaciones**. En la sobrecarga los métodos tienen diferentes tipos o cantidad de parámetros. El sistema sabe cual es el correcto por los parámetros que se le pasan.
...
> **links**  
> 📚 [Herencia en Java | Omega Knowledge](https://www.omega-knowledge.dev/curso/java/herencia-en-java)